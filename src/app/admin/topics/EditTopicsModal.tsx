...
interface Props {
  onClose: () => void;
  fetchTemas: () => void;
  currentTopic: any; // El tema actual que se va a editar
}

const EditTopicsModal: React.FC<Props> = ({ onClose, fetchTemas, currentTopic }) => {
  const t = useTranslations('general');
  const [temaNombre, setTemaNombre] = useState(currentTopic.nombre);
  const [temaDescripcion, setTemaDescripcion] = useState(currentTopic.descripcion);
  const [cursos, setCursos] = useState<Cursos[]>([]);
  const [selectedCurso, setSelectedCurso] = useState(currentTopic.curso?.nombre || ''); // Inicializamos con el curso actual
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await temaService.updateTema(currentTopic.id, {
        nombre: temaNombre,
        descripcion: temaDescripcion,
        nombre_curso: selectedCurso, // Enviar el nombre del curso actualizado
      });
    ...
    <ModalBase onClose={onClose} title={t('edit_details')} width={800} className="bg-white rounded-lg shadow-xl">
      <form onSubmit={handleSubmit} className="p-4">
        ...
        <div className="flex justify-end">
          ...
          >
            {t('save')}<CiCircleCheck className="inline ml-2 text-xl font-extrabold" />
          </button>
        </div>
      </form>
